...
*/
   ...
   void replay(std::function<bool()> shutdown)
   {
      ...
      ilog("replayed ${n} blocks in ${duration} seconds, ${mspb} ms/block",
           ("n", head->block_num - start_block_num)("duration", (end - start).count() / 1000000)("mspb", ((end - start).count() / 1000.0) / (head->block_num - start_block_num)));
      replaying = false;
    ...
    *  the pending block.
    */
   // 将交易信息写入到一个待打包的区块(pending_block)中
   transaction_trace_ptr push_transaction(const transaction_metadata_ptr &trx,
                                          fc::time_point deadline,
                                          uint32_t billed_cpu_time_us,
                                          bool explicit_billed_cpu_time = false)    // 显示计费cpu时间, controller 1277行调用
   {
      ...
      try
      {
         ...
         try
         {
            ...
            if (!trx->implicit)
            {
               ...
               pending->_pending_block_state->trxs.emplace_back(trx);      // pending中添加trx
            }
            else
            {
                    ...
                    const optional<block_id_type> &producer_block_id)
   {
      ...
      if (read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete)
      {
         ...
         )
         {
            ...
            if (!replaying)
            {
               ilog("promoting proposed schedule (set in block ${proposed_num}) to pending; current block: ${n} lib: ${lib} schedule: ${schedule} ",
                    ("proposed_num", *gpo.proposed_schedule_block_num)("n", pending->_pending_block_state->block_num)("lib", pending->_pending_block_state->dpos_irreversible_blocknum)("schedule", static_cast<producer_schedule_type>(gpo.proposed_schedule)));
            }
         ...
         try
         {
            // 触发 system 合约的 onblock action, 用于计算出块奖励和各节点的票数权重筛选出前21个节点
            auto onbtrx = std::make_shared<transaction_metadata>(get_on_block_transaction());      // 读取已经签名的交易的指针
            onbtrx->implicit = true;
            auto reset_in_trx_requiring_checks = fc::make_scoped_exit([old_value = in_trx_requiring_checks, this]() {
               in_trx_requiring_checks = old_value;
            });
            in_trx_requiring_checks = true;
            // 正式开始打包交易,截止日期,cpu用量,见1082行
...
// 这才是真正的区块生产函数,在 controller.cpp 中生产区块
...
sha256 controller::calculate_integrity_hash() const
{
   try
   {
...
bool controller::skip_db_sessions(block_status bs) const
{
   ...
   return consider_skipping && !my->conf.disable_replay_opts && !my->in_trx_requiring_checks;
}
...
void controller::validate_expiration(const transaction &trx) const
{
   try
   {
...
void controller::validate_tapos(const transaction &trx) const
{
   try
   {
      const auto &tapos_block_summary = db().get<block_summary_object>((uint16_t)trx.ref_block_num);
