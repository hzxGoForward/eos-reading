...
// 为投票抵押代币
   ...
   void system_contract::voteproducer( const account_name voter_name, const account_name proxy, const std::vector<account_name>& producers ) {
      require_auth( voter_name );  // 是否具有投票权限，包括是否具有权限，是否抵押过代币等
...
// 更新投票结果
   void system_contract::update_votes( const account_name voter_name, const account_name proxy, const std::vector<account_name>& producers, bool voting ) {
      ...
      auto voter = _voters.find(voter_name);// _voters是抵押的用户表，投票之前必须抵押过stake
       ...
       */
      if( voter->last_vote_weight <= 0.0 ) {   // 首次投票，修改全局投票状态
